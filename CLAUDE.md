...
### Build execution (v2/v3) — hands-off orchestrator
The build runs **hands-off**: this top-level session is the **orchestrator** and delegates to the subagents in `.claude/agents/` (implementer-core, implementer-app, test-runner, code-reviewer). Eddy is **not** in the per-story approval loop — the orchestrator, the `Stop`-hook gate (`.claude/hooks/gate.sh`), the brutal opus `code-reviewer`, and the GitHub `build-and-test` check are the approval authority. **The active backlog is [`docs/V3-PRD.md`](docs/V3-PRD.md) (epics 25–33; v1 epics 1–16 and v2 epics 17–24 are shipped).** Work one story at a time, one PR each, on feature branches only (never push/merge `main` directly). **Full procedure + safety rails + merge policy: [`docs/BUILD-WORKFLOW.md`](docs/BUILD-WORKFLOW.md) (ADR-0012).** ADR-0014 (coverage & clash) and ADR-0015 (day-count snapshots + effective-dated region moves) are written, so epics 28–30 may proceed. Pause for Eddy on unsettled product decisions, repeated gate failures, anything needing a new ADR, and **Epic 33 (go-live / WhosOff migration), which is human-gated — never run it autonomously.** The build runs the **`ponytail`** YAGNI skill (write the least code that meets each AC; never cut validation/security/a11y or the exhaustive `core/` tests) — install it in the build's Claude Code session per `docs/BUILD-WORKFLOW.md`.
