  ...
  return (
    <form action={action} style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: "var(--space-3)", alignItems: "end" }}>
      <label className="t-label" style={fieldCol}>Employee
        <select name="employeeId" className="input" required data-testid="ob-employee">{employees.map((e) => <option key={e.id} value={e.id}>{e.name}</option>)}</select>
      </label>
      <label className="t-label" style={fieldCol}>Leave type
        <select name="leaveTypeId" className="input" required data-testid="ob-type">{leaveTypes.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}</select>
      </label>
      ...
      <label className="t-label" style={fieldCol}>{mode === "MULTI" ? "Start" : "Date"}
        <input type="date" name="startDate" required className="input t-num" data-testid="ob-start" />
      </label>
      {mode === "MULTI" && (
        <label className="t-label" style={fieldCol}>End<input type="date" name="endDate" required className="input t-num" data-testid="ob-end" /></label>
      )}
      {mode === "HALF" && (
      ...
      {state && <span style={{ fontSize: "var(--text-xs)", color: state.ok ? "var(--success)" : "var(--danger)" }} data-testid="ob-result">{state.message}</span>}
    </form>
  );
}
