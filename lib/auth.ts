...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    ...
    async jwt({ token, profile }) {
      // `profile` is only present on initial sign-in. Resolve + link once; the stable
      // identity then rides on the token. Unprovisioned (null) is recovered at request
      // time by lib/rbac's email fallback, so no re-login is needed after provisioning.
      if (profile) {
        const p = profile as { email?: string; sub?: string };
